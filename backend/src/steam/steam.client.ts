...
async function getPlayerSummary(steamId: string): Promise<SteamPlayerSummary | null> {
    ...
    const res = await steamFetch<{ response: { players: SteamPlayerSummary[] } }>(url, 'getPlayerSummary');
...
async function getRecentPlayedGames(steamId: string, count: number): Promise<SteamRecentGame[]> {
    ...
    const res = await steamFetch<{ response: { games?: SteamRecentGame[] } }>(url, 'getRecentPlayedGames');
   
    return res.response.games ?? [];
}

async function getGameAssetImages(appIds: number[], l: string = 'schinese', countryCode: string = 'US'): Promise<SteamStoreItem[]> {
    ...
    const res = await steamFetch<{ response?: { store_items?: SteamStoreItem[] } }>(url, 'getGameAssetImages');
